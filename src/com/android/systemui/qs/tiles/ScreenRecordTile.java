...
public class ScreenRecordTile extends QSTileImpl<QSTile.BooleanState> implements RecordingController.RecordingStateChangeCallback {
    ...
    public QSTile.BooleanState newTileState() {
        ...
        booleanState.label = this.mContext.getString(C0015R$string.quick_settings_screen_record_label);
    ...
    public void handleUpdateState(QSTile.BooleanState booleanState, Object obj) {
        ...
        booleanState.label = this.mContext.getString(C0015R$string.quick_settings_screen_record_label);
        booleanState.icon = QSTileImpl.ResourceIcon.get(C0006R$drawable.ic_screenrecord);
        if (isRecording) {
            booleanState.secondaryLabel = this.mContext.getString(C0015R$string.quick_settings_screen_record_stop);
        } else if (isStarting) {
            booleanState.secondaryLabel = String.format("%d...", Integer.valueOf((int) Math.floorDiv(this.mMillisUntilFinished + 500, 1000)));
        } else {
            booleanState.secondaryLabel = this.mContext.getString(C0015R$string.quick_settings_screen_record_start);
        }
    ...
    public CharSequence getTileLabel() {
        return this.mContext.getString(C0015R$string.quick_settings_screen_record_label);
    }
    ...
    public void lambda$handleClick$0() {
        ...
        this.mKeyguardDismissUtil.executeWhenUnlocked(new ActivityStarter.OnDismissAction(this.mController.getPromptIntent()) { // from class: com.android.systemui.qs.tiles.-$$Lambda$ScreenRecordTile$qbmpsZNn23bn5rqlkcpltHLJl4w
