...
public class StorageNotification extends SystemUI {
            ...
            for (VolumeRecord volumeRecord : this.mStorageManager.getVolumeRecords()) {
                if (volumeRecord.getType() == 1) {
                    ...
                    if ((findVolumeByUuid == null || !findVolumeByUuid.isMountedWritable()) && !volumeRecord.isSnoozed()) {
                        String string = this.mContext.getString(17040124, volumeRecord.getNickname());
                        String string2 = this.mContext.getString(17040123);
                        Notification.Builder extend = new Notification.Builder(this.mContext, NotificationChannels.STORAGE).setSmallIcon(17302813).setColor(this.mContext.getColor(17170460)).setContentTitle(string).setContentText(string2).setContentIntent(buildForgetPendingIntent(volumeRecord)).setStyle(new Notification.BigTextStyle().bigText(string2)).setVisibility(1).setLocalOnly(true).setCategory("sys").setDeleteIntent(buildSnoozeIntent(fsUuid)).extend(new Notification.TvExtender());
    ...
    private void onDiskScannedInternal(DiskInfo diskInfo, int i) {
        ...
        String string = this.mContext.getString(17040154, diskInfo.getDescription());
        String string2 = this.mContext.getString(17040153, diskInfo.getDescription());
    ...
    private Notification onVolumeChecking(VolumeInfo volumeInfo) {
        ...
        return buildNotificationBuilder(volumeInfo, this.mContext.getString(17040121, disk.getDescription()), this.mContext.getString(17040120, disk.getDescription())).setCategory("progress").setOngoing(true).build();
    }
    ...
    private Notification onVolumeMounted(VolumeInfo volumeInfo) {
        ...
        if (!disk.isAdoptable() || findRecordByUuid.isInited()) {
            ...
            String string = this.mContext.getString(17040135, disk.getDescription());
            PendingIntent buildBrowsePendingIntent = buildBrowsePendingIntent(volumeInfo);
            Notification.Builder category = buildNotificationBuilder(volumeInfo, description, string).addAction(new Notification.Action(17302440, this.mContext.getString(17040119), buildBrowsePendingIntent)).addAction(new Notification.Action(17302422, this.mContext.getString(17040148), buildUnmountPendingIntent(volumeInfo))).setContentIntent(buildBrowsePendingIntent).setCategory("sys");
            if (disk.isAdoptable()) {
        ...
        String string2 = this.mContext.getString(17040131, disk.getDescription());
        ...
        return buildNotificationBuilder(volumeInfo, description2, string2).addAction(new Notification.Action(17302819, this.mContext.getString(17040122), buildInitPendingIntent)).addAction(new Notification.Action(17302422, this.mContext.getString(17040148), buildUnmountPendingIntent)).setContentIntent(buildInitPendingIntent).setDeleteIntent(buildSnoozeIntent(volumeInfo.getFsUuid())).build();
    }

    private Notification onVolumeEjecting(VolumeInfo volumeInfo) {
        DiskInfo disk = volumeInfo.getDisk();
        return buildNotificationBuilder(volumeInfo, this.mContext.getString(17040152, disk.getDescription()), this.mContext.getString(17040151, disk.getDescription())).setCategory("progress").setOngoing(true).build();
    }

    private Notification onVolumeUnmountable(VolumeInfo volumeInfo) {
        PendingIntent pendingIntent;
        DiskInfo disk = volumeInfo.getDisk();
        String string = this.mContext.getString(17040150, disk.getDescription());
        String string2 = this.mContext.getString(17040149, disk.getDescription());
        if (isAutomotive()) {
    ...
    private Notification onVolumeRemoved(VolumeInfo volumeInfo) {
        ...
        DiskInfo disk = volumeInfo.getDisk();
        return buildNotificationBuilder(volumeInfo, this.mContext.getString(17040134, disk.getDescription()), this.mContext.getString(17040133, disk.getDescription())).setCategory("err").build();
    }
    ...
    private Notification onVolumeBadRemoval(VolumeInfo volumeInfo) {
        ...
        DiskInfo disk = volumeInfo.getDisk();
        return buildNotificationBuilder(volumeInfo, this.mContext.getString(17040118, disk.getDescription()), this.mContext.getString(17040117, disk.getDescription())).setCategory("err").build();
    }
    ...
    private void onMoveProgress(MoveInfo moveInfo, int i, long j) {
        ...
        if (!TextUtils.isEmpty(moveInfo.label)) {
            str = this.mContext.getString(17040127, moveInfo.label);
        } else {
            str = this.mContext.getString(17040130);
        }
        if (j < 0) {
            charSequence = null;
        } else {
        ...
        Notification.Builder ongoing = new Notification.Builder(this.mContext, NotificationChannels.STORAGE).setSmallIcon(17302813).setColor(this.mContext.getColor(17170460)).setContentTitle(str).setContentText(charSequence).setContentIntent(pendingIntent).setStyle(new Notification.BigTextStyle().bigText(charSequence)).setVisibility(1).setLocalOnly(true).setCategory("progress").setProgress(100, i, false).setOngoing(true);
    ...
    private void onMoveFinished(MoveInfo moveInfo, int i) {
        ...
        if (i == -100) {
            str = this.mContext.getString(17040129);
            str2 = this.mContext.getString(17040128, bestVolumeDescription);
        } else {
            str = this.mContext.getString(17040126);
            str2 = this.mContext.getString(17040125);
        }
        ...
        Notification.Builder autoCancel = new Notification.Builder(this.mContext, NotificationChannels.STORAGE).setSmallIcon(17302813).setColor(this.mContext.getColor(17170460)).setContentTitle(str).setContentText(str2).setContentIntent(pendingIntent).setStyle(new Notification.BigTextStyle().bigText(str2)).setVisibility(1).setLocalOnly(true).setCategory("sys").setAutoCancel(true);
    ...
    private PendingIntent buildBrowsePendingIntent(VolumeInfo volumeInfo) {
        ...
        try {
            Intent buildOneplusBrowseIntentForUser = buildOneplusBrowseIntentForUser(volumeInfo);
            return PendingIntent.getActivityAsUser(this.mContext, volumeInfo.getId().hashCode(), buildOneplusBrowseIntentForUser, 268435456, null, UserHandle.CURRENT);
        } finally {
            StrictMode.setVmPolicy(allowVmViolations);
        }
    }

    public Intent buildOneplusBrowseIntentForUser(VolumeInfo volumeInfo) {
        Uri parse = Uri.parse("opfile://filemanager.oneplus.com/browse?page=storage&directory=" + volumeInfo.getPath());
