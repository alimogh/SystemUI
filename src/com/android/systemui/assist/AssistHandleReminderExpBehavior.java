...
public final class AssistHandleReminderExpBehavior implements AssistHandleBehaviorController.BehaviorController {
    ...
    private final BootCompleteCache.BootCompleteListener mBootCompleteListener = new BootCompleteCache.BootCompleteListener() { // from class: com.android.systemui.assist.AssistHandleReminderExpBehavior.6
        ...
        public void onBootComplete() {
            ...
            OpUtils.updateSupportAssistantGestureState(AssistHandleReminderExpBehavior.this.mContext);
        }
    };
    ...
    private final SysUiState.SysUiStateCallback mSysUiStateCallback = new SysUiState.SysUiStateCallback() { // from class: com.android.systemui.assist.-$$Lambda$AssistHandleReminderExpBehavior$V4NCzVQFEFR-zsFBikU8WKQiVok
    ...
    public void onModeActivated(Context context, AssistHandleCallbacks assistHandleCallbacks) {
        ...
        OpUtils.updateSupportAssistantGestureState(this.mContext);
    ...
    private void callbackForLearnedState(boolean z) {
        if (this.mAssistHandleCallbacks != null) {
            boolean shouldHideCornerHandleView = shouldHideCornerHandleView();
            if (!isFullyAwake() || this.mIsNavBarHidden || this.mOnLockscreen || !getShowWhenTaught() || shouldHideCornerHandleView) {
                ...
                if (this.mLearnedHintLastShownEpochDay < epochDay) {
                    ...
                    Log.d("AssistHandleReminder", "callbackForLearnedState supportAssistantGesture :" + OpUtils.sIsSupportAssistantGesture + ", isH2:" + OpUtils.isHydrogen());
                    if (!OpUtils.sIsSupportAssistantGesture || OpUtils.isHydrogen()) {
                        this.mAssistHandleCallbacks.hide();
                    } else {
                        this.mAssistHandleCallbacks.showAndGo();
                    }
    ...
    private void callbackForUnlearnedState() {
        Log.d("AssistHandleReminder", "callbackForUnlearnedState supportAssistantGesture :" + OpUtils.sIsSupportAssistantGesture + ", isH2:" + OpUtils.isHydrogen());
        if (this.mAssistHandleCallbacks != null) {
            if (!OpUtils.sIsSupportAssistantGesture || OpUtils.isHydrogen()) {
                this.mAssistHandleCallbacks.hide();
                return;
            ...
            boolean shouldHideCornerHandleView = shouldHideCornerHandleView();
            Log.d("AssistHandleReminder", "callbackForUnlearnedState isFullyAwake :" + isFullyAwake() + ", mIsNavBarHidden:" + this.mIsNavBarHidden + ",isSuppressed:" + isSuppressed() + ", mOnLockscreen:" + this.mOnLockscreen + ", mIsLauncherShowing:" + this.mIsLauncherShowing + ", shouldHideCornerHint:" + shouldHideCornerHandleView);
            if (!isFullyAwake() || this.mIsNavBarHidden || isSuppressed() || shouldHideCornerHandleView) {
                this.mAssistHandleCallbacks.hide();
    ...
    private /* synthetic */ void lambda$updateLearningStatus$0$AssistHandleReminderExpBehavior() {
    ...
    private boolean shouldHideCornerHandleView() {
        if (this.mStatusBar == null) {
            this.mStatusBar = OpLsState.getInstance().getPhoneStatusBar();
        }
        StatusBar statusBar = this.mStatusBar;
        return statusBar != null && (statusBar.getNavigationBarHiddenMode() == 1 || !EdgeBackGestureHandler.sSideGestureEnabled);
    }
}
