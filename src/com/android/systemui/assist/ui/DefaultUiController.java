...
public class DefaultUiController implements AssistManager.UiController {
    ...
    public DefaultUiController(Context context, AssistLogger assistLogger) {
        ...
        InvocationLightsView invocationLightsView = (InvocationLightsView) LayoutInflater.from(context).inflate(C0011R$layout.invocation_lights, (ViewGroup) this.mRoot, false);
    ...
    public void onInvocationProgress(int i, float f) {
        ...
        if (f == 1.0f) {
            Log.d("DefaultUiController", "onInvocationProgress 1, type:" + i);
            animateInvocationCompletion(i, 0.0f);
        } else if (f == 0.0f) {
            Log.d("DefaultUiController", "onInvocationProgress 0, type:" + i);
            hide();
        } else {
            if (!z) {
                attach();
    ...
    public void onGestureCompletion(float f) {
        Log.d("DefaultUiController", "onGestureCompletion v:" + f);
    ...
    public void onOverlayChanged() {
        if (this.mRoot != null && this.mInvocationLightsView != null) {
            Log.d("DefaultUiController", "onOverlayChanged to reinflate InvocationLightsView");
            this.mRoot.removeView(this.mInvocationLightsView);
            InvocationLightsView invocationLightsView = (InvocationLightsView) LayoutInflater.from(this.mRoot.getContext()).inflate(C0011R$layout.invocation_lights, (ViewGroup) this.mRoot, false);
            this.mInvocationLightsView = invocationLightsView;
            this.mRoot.addView(invocationLightsView);
        }
    }
    ...
    private void animateInvocationCompletion(int i, float f) {
        ...
        this.mInvocationAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener(i) { // from class: com.android.systemui.assist.ui.-$$Lambda$DefaultUiController$DsyFMixn8vpgo7pkqARg9d_ZEVw
