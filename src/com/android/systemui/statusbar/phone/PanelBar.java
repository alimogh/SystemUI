...
public abstract class PanelBar extends FrameLayout {
    ...
    protected PanelViewController mPanel;
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        if (!panelEnabled()) {
            if (motionEvent.getAction() == 0) {
                Log.v(str, String.format("onTouch: all panels disabled, ignoring touch at (%d,%d)", Integer.valueOf((int) motionEvent.getX()), Integer.valueOf((int) motionEvent.getY())));
            }
            return false;
        }
        if (motionEvent.getAction() == 0) {
            PanelViewController panelViewController = this.mPanel;
            if (panelViewController == null) {
                Log.v(str, String.format("onTouch: no panel for touch at (%d,%d)", Integer.valueOf((int) motionEvent.getX()), Integer.valueOf((int) motionEvent.getY())));
                return true;
            } else if (!panelViewController.isEnabled()) {
                Log.v(str, String.format("onTouch: panel (%s) is disabled, ignoring touch at (%d,%d)", panelViewController, Integer.valueOf((int) motionEvent.getX()), Integer.valueOf((int) motionEvent.getY())));
                return true;
            }
        }
    ...
    public void panelExpansionChanged(float f, boolean z) {
        ...
        if (!Float.isNaN(f)) {
            ...
            Log.d(TAG, "panelExpansionChanged isNaN frac " + f + " expanded " + z + Debug.getCallers(10));
