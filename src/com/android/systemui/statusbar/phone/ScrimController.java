...
public class ScrimController implements ViewTreeObserver.OnPreDrawListener, ColorExtractor.OnColorsChangedListener, Dumpable {
    ...
    private static final int TAG_END_ALPHA = C0008R$id.scrim_alpha_end;
    static final int TAG_KEY_ANIM = C0008R$id.scrim;
    private static final int TAG_START_ALPHA = C0008R$id.scrim_alpha_start;
    ...
    public ScrimController(LightBarController lightBarController, DozeParameters dozeParameters, AlarmManager alarmManager, final KeyguardStateController keyguardStateController, DelayedWakeLock.Builder builder, Handler handler, KeyguardUpdateMonitor keyguardUpdateMonitor, SysuiColorExtractor sysuiColorExtractor, DockManager dockManager, BlurUtils blurUtils) {
        ...
        this.mScrimStateListener = new TriConsumer() { // from class: com.android.systemui.statusbar.phone.-$$Lambda$v3pYAGeeZEy0j9LKp92o1adNfrk
            public final void accept(Object obj, Object obj2, Object obj3) {
                LightBarController.this.setScrimState((ScrimState) obj, ((Float) obj2).floatValue(), (ColorExtractor.GradientColors) obj3);
            }
        };
        this.mDefaultScrimAlpha = blurUtils.supportsBlursOnWindows() ? 0.54f : 0.65f;
        ...
        if (scrimState != this.mState) {
            ...
            if (scrimState != ScrimState.UNINITIALIZED) {
                ...
                if (shouldFadeAwayWallpaper()) {
                    DejankUtils.postAfterTraversal(new Runnable() { // from class: com.android.systemui.statusbar.phone.-$$Lambda$ScrimController$YQJRwwTLFgaOweq9aHvS8f9csz8
                    ...
                    DejankUtils.postAfterTraversal(new Runnable() { // from class: com.android.systemui.statusbar.phone.-$$Lambda$0ZxUFLvlsGlm9ET2o7nSDW8wc5w
                        @Override // java.lang.Runnable
                        public final void run() {
                ...
                if (this.mKeyguardUpdateMonitor.needsSlowUnlockTransition() && this.mState == ScrimState.UNLOCKED) {
                    int i = SystemProperties.getInt("debug.reboot.scrim.anim", 0);
                    if (i != 0) {
                        if (i == 1) {
                        ...
                        this.mAnimationDelay = 100;
                        if (this.mKeyguardUpdateMonitor.isLaunchingEmergencyCall()) {
                            this.mAnimationDelay = 0;
                        }
                    }
    ...
    private void setOrAdaptCurrentAnimation(View view) {
        ...
        if (scrimState == ScrimState.UNLOCKED || scrimState == ScrimState.BUBBLE_EXPANDED) {
            updateScrimColor(view, currentScrimAlpha, -16777216);
        } else {
    ...
    private void applyAndDispatchExpansion() {
        ...
        if (!this.mUpdatePending) {
            ...
            if (this.mWallpaperVisibilityTimedOut) {
                this.mWallpaperVisibilityTimedOut = false;
                DejankUtils.postAfterTraversal(new Runnable() { // from class: com.android.systemui.statusbar.phone.-$$Lambda$ScrimController$qcAzw93VG0gxAU1AfapPWpIf3aU
                    @Override // java.lang.Runnable
                    public final void run() {
    ...
    public void updateScrims() {
        if (Build.DEBUG_ONEPLUS) {
            Log.d("ScrimController", "updateScrims: , mState = " + this.mState + ", anim = " + this.mAnimateChange + ", mTracking = " + this.mTracking + ", Force = " + this.mForceHideScrims + ", mDarkenWhileDragging = " + this.mDarkenWhileDragging + ", mColors = " + this.mColors);
        }
        boolean z = true;
        ...
        if (Build.DEBUG_ONEPLUS) {
            Log.d("ScrimController", "updateScrims: inFrontAlpha= " + this.mInFrontAlpha + ", behindAlpha= " + this.mBehindAlpha + ", bubbleAlpha= " + this.mBubbleAlpha);
        }
        if (this.mForceHideScrims) {
            this.mAnimateChange = false;
            setScrimAlpha(this.mScrimInFront, 0.0f);
            setScrimAlpha(this.mScrimBehind, 0.0f);
            setScrimAlpha(this.mScrimForBubble, 0.0f);
        } else {
    ...
    private void startScrimAnimation(final View view, float f) {
        ...
        ofFloat.addUpdateListener(new ValueAnimator.AnimatorUpdateListener(view, view instanceof ScrimView ? ((ScrimView) view).getTint() : 0) { // from class: com.android.systemui.statusbar.phone.-$$Lambda$ScrimController$pQ1ZzyQHHAbZJylpLDQQk40ggTo
    ...
    public /* synthetic */ void lambda$startScrimAnimation$2$ScrimController(View view, int i, ValueAnimator valueAnimator) {
        ...
        dispatchScrimsVisible();
        if (this.mQSExpend && this.mState == ScrimState.UNLOCKED) {
    ...
    private void updateScrim(ScrimView scrimView, float f) {
        ...
        if (this.mPendingFrameCallback == null) {
            ...
            ScrimState scrimState = this.mState;
            updateScrimColor(scrimView, f, (scrimState == ScrimState.UNLOCKED || scrimState == ScrimState.BUBBLE_EXPANDED) ? -16777216 : getCurrentScrimTint(scrimView));
        }
    }
    ...
    private void blankDisplay() {
        ...
        $$Lambda$ScrimController$ag08GXJhpSWypcA8hrLE9y1Zo r0 = new Runnable() { // from class: com.android.systemui.statusbar.phone.-$$Lambda$ScrimController$ag-08GXJhpSWypcA8-hrLE9y1Zo
            @Override // java.lang.Runnable
            public final void run() {
    ...
    public /* synthetic */ void lambda$blankDisplay$4$ScrimController() {
        ...
        this.mBlankingTransitionRunnable = new Runnable() { // from class: com.android.systemui.statusbar.phone.-$$Lambda$ScrimController$naUAB1OlntOHTCtGQlLQ0dSkAuw
            @Override // java.lang.Runnable
            public final void run() {
    ...
    public void dump(FileDescriptor fileDescriptor, PrintWriter printWriter, String[] strArr) {
        ...
        printWriter.print(Integer.toHexString(getBackgroundColor()));
    }
    ...
    public void setExpansionAffectsAlpha(boolean z) {
        ...
        NotificationPanelViewController panelController = OpLsState.getInstance().getPhoneStatusBar().getPanelController();
        if (biometricUnlockController.unlockedByWakeAndUnlock() && panelController.getView().getVisibility() == 0) {
            Log.d("ScrimController", "setExpansionAffectsAlpha: skip because we are unlocked by wake and unlock.");
        } else if (z) {
            applyAndDispatchExpansion();
        }
    }
    ...
    public class KeyguardVisibilityCallback extends KeyguardUpdateMonitorCallback {
        ...
        public void onQSExpendChanged(boolean z) {
            if (Build.DEBUG_ONEPLUS) {
                Log.i("ScrimController", "onQSExpendChanged, expend:" + z + ", mState:" + ScrimController.this.mState + ", isRecentUnlockBiometricFinger:" + OpUtils.isRecentUnlockBiometricFinger() + ", BiometricUnlockController().getMode:" + OpLsState.getInstance().getBiometricUnlockController().getMode());
            }
            ScrimController.this.mQSExpend = z;
        }
    }
    ...
    public void forceHideScrims(boolean z, boolean z2) {
        ...
        OpLsState.getInstance().getPhoneStatusBar().shouldForceHideWallpaper(z);
        scheduleUpdate();
    }

    public void resetForceHide() {
        OpLsState.getInstance().getPhoneStatusBar().shouldForceHideWallpaper(false);
