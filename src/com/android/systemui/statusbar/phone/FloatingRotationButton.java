...
public class FloatingRotationButton implements RotationButton {
    ...
    FloatingRotationButton(Context context) {
        ...
        KeyButtonView keyButtonView = (KeyButtonView) LayoutInflater.from(this.mContext).inflate(C0011R$layout.rotate_suggestion, (ViewGroup) null);
        this.mKeyButtonView = keyButtonView;
        keyButtonView.setVisibility(0);
        Resources resources = this.mContext.getResources();
        this.mDiameter = resources.getDimensionPixelSize(C0005R$dimen.floating_rotation_button_diameter);
        this.mMargin = Math.max(resources.getDimensionPixelSize(C0005R$dimen.floating_rotation_button_min_margin), resources.getDimensionPixelSize(C0005R$dimen.rounded_corner_content_padding));
    }
    ...
    public boolean show() {
        ...
        layoutParams.setFitInsetsTypes(WindowInsets.Type.statusBars() | WindowInsets.Type.navigationBars());
    ...
    public KeyButtonDrawable getImageDrawable() {
        ...
        int themeAttr = Utils.getThemeAttr(contextThemeWrapper, C0002R$attr.darkIconTheme);
        ContextThemeWrapper contextThemeWrapper2 = new ContextThemeWrapper(contextThemeWrapper, Utils.getThemeAttr(contextThemeWrapper, C0002R$attr.lightIconTheme));
        int colorAttrDefaultColor = Utils.getColorAttrDefaultColor(new ContextThemeWrapper(contextThemeWrapper, themeAttr), C0002R$attr.singleToneColor);
        return KeyButtonDrawable.create(contextThemeWrapper2, Utils.getColorAttrDefaultColor(contextThemeWrapper2, C0002R$attr.singleToneColor), colorAttrDefaultColor, C0006R$drawable.ic_sysbar_rotate_button, false, Color.valueOf((float) Color.red(colorAttrDefaultColor), (float) Color.green(colorAttrDefaultColor), (float) Color.blue(colorAttrDefaultColor), 0.92f));
    }
