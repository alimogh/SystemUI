...
public class LightBarTransitionsController implements Dumpable, CommandQueue.Callbacks, StatusBarStateController.StateListener {
    ...
    public LightBarTransitionsController(Context context, DarkIntensityApplier darkIntensityApplier, CommandQueue commandQueue) {
        ...
        Log.d("LightBar", "TransitionsController:" + this.mApplier + " init:");
    }
        ...
        if (!this.mKeyguardStateController.isKeyguardGoingAway() || z) {
            if (this.mTransitionPending && this.mTintChangePending) {
                ...
                if (Build.DEBUG_ONEPLUS) {
                    Log.d("LightBar", "TransitionsController:" + this.mApplier + " TransitionStarting, " + this.mPendingDarkIntensity + ", " + j2 + ", " + z + ", " + this.mNextDarkIntensity);
                }
    ...
    public void setIconsDark(boolean z, boolean z2) {
        if (Build.DEBUG_ONEPLUS) {
            Log.d("LightBar", "TransitionsController:" + this.mApplier + " setIconsDark, " + z + "," + z2 + "," + this.mTransitionPending + "," + this.mTransitionDeferring + "," + this.mNextDarkIntensity + " mPendingDarkIntensity:" + this.mPendingDarkIntensity);
        }
        float f = 1.0f;
        if (!z2) {
        ...
        if (this.mBypassTransition) {
            ...
            this.mTransitionPending = false;
            Log.i("LightBar", "Force disable TransitionPending");
        }
    }
    ...
    private void animateIconTint(float f, long j, long j2) {
        if (this.mNextDarkIntensity != f) {
            ...
            ofFloat.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() { // from class: com.android.systemui.statusbar.phone.-$$Lambda$LightBarTransitionsController$PJRveQsGC7aANrqdSv3tRYb3x7c
