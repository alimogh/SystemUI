            ...
            if (notification.isColorizedMedia()) {
                ...
                try {
                    ...
                    if (!isWhiteOrBlack(findBackgroundSwatch.getHsl())) {
                        generateArtworkPaletteBuilder.addFilter(new Palette.Filter(findBackgroundSwatch.getHsl()[0]) { // from class: com.android.systemui.statusbar.notification.-$$Lambda$MediaNotificationProcessor$jNuRDwOMbOj8fwROH917lxaryoM
                            ...
                            public final boolean isAllowed(int i3, float[] fArr) {
                                return MediaNotificationProcessor.lambda$processNotification$1(this.f$0, i3, fArr);
                            }
                        });
                    }
                    ...
                    Log.d("MediaNotificationProcessor", e.getMessage());
                    int color = this.mContext.getColor(C0004R$color.notification_material_background_color);
                ...
                i = this.mContext.getColor(C0004R$color.notification_material_background_color);
            }
            ImageGradientColorizer imageGradientColorizer2 = this.mColorizer;
            if (this.mContext.getResources().getConfiguration().getLayoutDirection() != 1) {
                z = false;
            }
            builder.setLargeIcon(Icon.createWithBitmap(imageGradientColorizer2.colorize(loadDrawable, i, z)));
        }
    }
