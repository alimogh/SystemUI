...
public class InstantAppNotifier extends SystemUI implements CommandQueue.Callbacks, KeyguardStateController.Callback {
    ...
    private final Consumer<Boolean> mDockedListener = new Consumer() { // from class: com.android.systemui.statusbar.notification.-$$Lambda$InstantAppNotifier$xtVHKTfUxObWlx6TMvVdHY7npdU
        @Override // java.util.function.Consumer
        public final void accept(Object obj) {
            InstantAppNotifier.this.lambda$new$0$InstantAppNotifier((Boolean) obj);
        }
    };
    ...
    public /* synthetic */ void lambda$new$0$InstantAppNotifier(Boolean bool) {
        this.mDockedStackExists = bool.booleanValue();
        updateForegroundInstantApps();
        this.mOpNotificationController.setIsInSplitScreen(bool.booleanValue());
    }
    ...
    public void start() {
        ...
        this.mDivider.registerInSplitScreenListener(this.mDockedListener);
    ...
    private void updateForegroundInstantApps() {
        this.mUiBgExecutor.execute(new Runnable((NotificationManager) this.mContext.getSystemService(NotificationManager.class), AppGlobals.getPackageManager()) { // from class: com.android.systemui.statusbar.notification.-$$Lambda$InstantAppNotifier$-jG9Ev-YNY9H1cwQp_C5lfrjo3s
    ...
    public /* synthetic */ void lambda$updateForegroundInstantApps$2$InstantAppNotifier(NotificationManager notificationManager, IPackageManager iPackageManager) {
        ...
        arraySet.forEach(new Consumer(notificationManager) { // from class: com.android.systemui.statusbar.notification.-$$Lambda$InstantAppNotifier$Z2PXeDEr90IgG8nCbYZnqFPYPNc
            public final /* synthetic */ NotificationManager f$1;
        ...
        if (stackInfo != null) {
            try {
                if (stackInfo.topActivity != null) {
                    ...
                    if (!arraySet.remove(new Pair(packageName, Integer.valueOf(stackInfo.userId))) && (applicationInfo = iPackageManager.getApplicationInfo(packageName, 8192, stackInfo.userId)) != null && applicationInfo.isInstantApp()) {
    ...
    private void postInstantAppNotif(String str, int i, ApplicationInfo applicationInfo, NotificationManager notificationManager, int i2) {
        ...
        bundle.putString("android.substName", this.mContext.getString(C0015R$string.instant_apps));
        this.mCurrentNotifs.add(new Pair<>(str, Integer.valueOf(i)));
        String string = this.mContext.getString(C0015R$string.instant_apps_help_url);
        boolean z = !string.isEmpty();
        Context context = this.mContext;
        if (z) {
            i3 = C0015R$string.instant_apps_message_with_help;
        } else {
            i3 = C0015R$string.instant_apps_message;
        }
        String string2 = context.getString(i3);
        UserHandle of = UserHandle.of(i);
        Notification.Action build = new Notification.Action.Builder((Icon) null, this.mContext.getString(C0015R$string.app_info), PendingIntent.getActivityAsUser(this.mContext, 0, new Intent("android.settings.APPLICATION_DETAILS_SETTINGS").setData(Uri.fromParts("package", str, null)), 67108864, null, of)).build();
        if (z) {
        ...
        if (taskIntent == null || !taskIntent.isWebIntent()) {
            ...
            builder = builder2;
            builder.addAction(new Notification.Action.Builder((Icon) null, this.mContext.getString(C0015R$string.go_to_web), PendingIntent.getActivityAsUser(this.mContext, 0, addCategory.addCategory("unique:" + System.currentTimeMillis()).putExtra("android.intent.extra.PACKAGE_NAME", applicationInfo.packageName).putExtra("android.intent.extra.VERSION_CODE", applicationInfo.versionCode & Integer.MAX_VALUE).putExtra("android.intent.extra.LONG_VERSION_CODE", applicationInfo.longVersionCode).putExtra("android.intent.extra.INSTANT_APP_FAILURE", activityAsUser), 67108864, null, of)).build());
        }
        Notification.Builder color = builder.addExtras(bundle).addAction(action).setContentIntent(pendingIntent2).setColor(this.mContext.getColor(C0004R$color.instant_apps_color));
        Context context2 = this.mContext;
        notificationManager.notifyAsUser(str, 7, color.setContentTitle(context2.getString(C0015R$string.instant_apps_title, applicationInfo.loadLabel(context2.getPackageManager()))).setLargeIcon(Icon.createWithResource(str, applicationInfo.icon)).setSmallIcon(Icon.createWithResource(this.mContext.getPackageName(), C0006R$drawable.instant_icon)).setContentText(string2).setStyle(new Notification.BigTextStyle().bigText(string2)).setOngoing(true).build(), new UserHandle(i));
    }
