...
public class AmbientState {
    ...
    public void setOverScrollAmount(float f, boolean z) {
        if (Float.isNaN(f)) {
            String str = TAG;
            Log.i(str, "setOverScrollAmount amount:" + f + " onTop:" + z + Debug.getCallers(10));
        }
        if (z) {
    ...
    public int getInnerHeight(boolean z) {
        ...
        int min = Math.min(Math.max(Math.min(this.mLayoutHeight, this.mMaxLayoutHeight) - this.mTopPadding, this.mLayoutMinHeight), (this.mLayoutMaxHeight - this.mTopPadding) - this.mPaddingBottom);
        if (z) {
            return min;
        }
        float f = (float) min;
