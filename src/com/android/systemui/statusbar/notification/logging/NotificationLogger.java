...
public class NotificationLogger implements StatusBarStateController.StateListener {
    ...
    private void logNotificationVisibilityChanges(Collection<NotificationVisibility> collection, Collection<NotificationVisibility> collection2) {
        if (!collection.isEmpty() || !collection2.isEmpty()) {
            this.mUiBgExecutor.execute(new Runnable(cloneVisibilitiesAsArr(collection), cloneVisibilitiesAsArr(collection2)) { // from class: com.android.systemui.statusbar.notification.logging.-$$Lambda$NotificationLogger$e3uK-rBablkegG4HWqs1WzubMAs
        ...
        private void maybeNotifyOnNotificationExpansionChanged(String str, State state) {
            if (state.isFullySet() && state.mIsVisible.booleanValue()) {
                ...
                if (bool == null || state.mIsExpanded != bool) {
                    ...
                    this.mUiBgExecutor.execute(new Runnable(str, new State(state)) { // from class: com.android.systemui.statusbar.notification.logging.-$$Lambda$NotificationLogger$ExpansionStateLogger$2Eiyi73G6QB8CNmBwaixENnG5Co
