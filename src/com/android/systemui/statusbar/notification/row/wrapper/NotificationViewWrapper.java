...
public abstract class NotificationViewWrapper implements TransformableView {
    ...
    public void onReinflated() {
        if (this.mRow.getEntry() == null || this.mRow.getEntry().getSbn() == null || this.mRow.getEntry().getSbn().getNotification() == null || this.mRow.getEntry().getSbn().getNotification().extras == null || !this.mRow.getEntry().getSbn().getNotification().extras.getBoolean("op_disable_inversion")) {
        ...
        Log.d("NotificationViewWrapper", "needsInversion, skip for key: " + this.mRow.getEntry().getKey());
    }
    ...
    public boolean needsInversion(int i, View view) {
        ...
        if ((this.mRow.getEntry() == null || this.mRow.getEntry().getSbn() == null) ? false : this.mOpNotificationController.forceInversion(this.mRow.getEntry().getSbn().getPackageName())) {
            Log.d("NotificationViewWrapper", "froceInversion for " + this.mRow.getEntry().getSbn().getPackageName());
            return true;
        } else if (this.mRow.getEntry().targetSdk >= 29) {
            Log.d("NotificationViewWrapper", "no needsInversion, targetSdk over Q");
            return false;
        } else if (this.mRow.getEntry() == null || this.mRow.getEntry().getSbn() == null || this.mRow.getEntry().getSbn().getNotification() == null || this.mRow.getEntry().getSbn().getNotification().extras == null || !this.mRow.getEntry().getSbn().getNotification().extras.getBoolean("op_disable_inversion")) {
            ...
            if (fArr[1] != 0.0f) {
                Log.d("NotificationViewWrapper", "no needs Inversion" + this.mRow.getEntry().getKey() + ",skip for backgournd color: " + Integer.toHexString(i));
                return false;
            }
        ...
        for (int i2 = 0; i2 < viewGroup.getChildCount(); i2++) {
            ...
            if (childAt instanceof TextView) {
                if (ColorUtils.calculateContrast(((TextView) childAt).getCurrentTextColor(), backgroundColor) < 3.0d) {
                    childAt.measure(0, 0);
                    if (childAt.getMeasuredWidth() > 4 && childAt.getMeasuredHeight() > 4) {
                        return true;
                    }
                    Log.d("NotificationViewWrapper", "childrenNeedInversion visible area width " + childAt.getMeasuredWidth() + " height " + childAt.getMeasuredHeight());
                } else {
                    continue;
                }
    ...
    public int resolveBackgroundColor() {
        ...
        return this.mView.getContext().getColor(17170888);
    }
