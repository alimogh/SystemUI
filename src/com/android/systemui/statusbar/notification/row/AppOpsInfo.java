...
public class AppOpsInfo extends LinearLayout implements NotificationGuts.GutsContent {
    ...
    private View.OnClickListener mOnOk = new View.OnClickListener() { // from class: com.android.systemui.statusbar.notification.row.-$$Lambda$AppOpsInfo$zS48CwL7b6UcUOuxgx7Zkw4dC1A
    ...
    private void bindHeader() {
        ...
        ((ImageView) findViewById(C0008R$id.pkgicon)).setImageDrawable(drawable);
        ((TextView) findViewById(C0008R$id.pkgname)).setText(this.mAppName);
    }

    private void bindPrompt() {
        ((TextView) findViewById(C0008R$id.prompt)).setText(getPrompt());
    }

    private void bindButtons() {
        findViewById(C0008R$id.settings).setOnClickListener(new View.OnClickListener() { // from class: com.android.systemui.statusbar.notification.row.-$$Lambda$AppOpsInfo$MC_PUe5w52BX3b0kt9URHDzbSUA
            @Override // android.view.View.OnClickListener
            public final void onClick(View view) {
                AppOpsInfo.this.lambda$bindButtons$1$AppOpsInfo(view);
            }
        });
        TextView textView = (TextView) findViewById(C0008R$id.ok);
    ...
    private String getPrompt() {
        ...
        if (this.mAppOps.size() == 1) {
            if (this.mAppOps.contains(26)) {
                return ((LinearLayout) this).mContext.getString(C0015R$string.appops_camera);
            }
            if (this.mAppOps.contains(27)) {
                return ((LinearLayout) this).mContext.getString(C0015R$string.appops_microphone);
            }
            return ((LinearLayout) this).mContext.getString(C0015R$string.appops_overlay);
        } else if (this.mAppOps.size() != 2) {
            return ((LinearLayout) this).mContext.getString(C0015R$string.appops_camera_mic_overlay);
        } else {
            if (!this.mAppOps.contains(26)) {
                return ((LinearLayout) this).mContext.getString(C0015R$string.appops_mic_overlay);
            }
            if (this.mAppOps.contains(27)) {
                return ((LinearLayout) this).mContext.getString(C0015R$string.appops_camera_mic);
            }
            return ((LinearLayout) this).mContext.getString(C0015R$string.appops_camera_overlay);
        }
    }
