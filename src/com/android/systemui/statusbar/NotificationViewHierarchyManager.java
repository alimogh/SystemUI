...
public class NotificationViewHierarchyManager extends OpNotificationViewHierarchyManager implements DynamicPrivacyController.Listener {
    ...
    public NotificationViewHierarchyManager(Context context, Handler handler, NotificationLockscreenUserManager notificationLockscreenUserManager, NotificationGroupManager notificationGroupManager, VisualStabilityManager visualStabilityManager, StatusBarStateController statusBarStateController, NotificationEntryManager notificationEntryManager, KeyguardBypassController keyguardBypassController, BubbleController bubbleController, DynamicPrivacyController dynamicPrivacyController, ForegroundServiceSectionController foregroundServiceSectionController, DynamicChildBindController dynamicChildBindController, LowPriorityInflationHelper lowPriorityInflationHelper) {
        ...
        this.mAlwaysExpandNonGroupedNotification = context.getResources().getBoolean(C0003R$bool.config_alwaysExpandNonGroupedNotifications);
    ...
    public void updateNotificationViews() {
        ...
        while (true) {
            ...
            if (!notificationEntry.isRowDismissed() && !notificationEntry.isRowRemoved() && !this.mBubbleController.isBubbleNotificationSuppressedFromShade(notificationEntry) && !this.mFgsSectionController.hasEntry(notificationEntry)) {
                ...
                boolean z3 = this.mLockscreenUserManager.isSecure() && this.mEntryManager.isLocked(notificationEntry.getKey());
                boolean z4 = this.mLockscreenUserManager.needsRedaction(notificationEntry) || z3;
                notificationEntry.setSensitive((z2 || z3) && z4, (isLockscreenPublicMode && !this.mLockscreenUserManager.userAllowsPrivateNotificationsInPublic(currentUserId)) || z3);
                notificationEntry.getRow().setNeedsRedaction(z4 || z3);
                notificationEntry.getRow().setContentHidden(z3);
                if (z3) {
    ...
    private void updateRowStatesInternal() {
        ...
        while (!stack.isEmpty()) {
            ...
            if (!z) {
                entry.getSbn().getNotification().extras.getBoolean("op_always_expanded");
                expandableNotificationRow.setSystemExpanded(this.mAlwaysExpandNonGroupedNotification || (i2 == 0 && !isChildInGroupWithSummary && !isChildInGroupWithSummary && !expandableNotificationRow.isLowPriority()));
            }
    ...
    public boolean shouldHideSensitive(NotificationEntry notificationEntry, int i) {
        int currentUserId = this.mLockscreenUserManager.getCurrentUserId();
        boolean isLockscreenPublicMode = this.mLockscreenUserManager.isLockscreenPublicMode(currentUserId);
        boolean z = this.mLockscreenUserManager.isSecure() && this.mEntryManager.isLocked(notificationEntry.getKey());
        ...
        return (z2 && (this.mLockscreenUserManager.needsRedaction(notificationEntry) || z)) || (isLockscreenPublicMode && !this.mLockscreenUserManager.userAllowsPrivateNotificationsInPublic(currentUserId)) || z;
    }
}
