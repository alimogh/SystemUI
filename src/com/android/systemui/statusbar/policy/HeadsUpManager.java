...
import com.oneplus.systemui.statusbar.policy.OpHeadsUpManager;
import java.io.FileDescriptor;
import java.io.PrintWriter;
import java.util.HashSet;
import java.util.Iterator;
public abstract class HeadsUpManager extends OpHeadsUpManager {
    ...
    public HeadsUpManager(final Context context) {
        ...
        this.mMinimumDisplayTime = resources.getInteger(C0009R$integer.heads_up_notification_minimum_time);
        this.mAutoDismissNotificationDecay = resources.getInteger(C0009R$integer.heads_up_notification_decay);
        this.mTouchAcceptanceDelay = resources.getInteger(C0009R$integer.touch_acceptance_delay);
        this.mSnoozedPackages = new ArrayMap<>();
        this.mSnoozeLengthMs = Settings.Global.getInt(context.getContentResolver(), "heads_up_snooze_length_ms", resources.getInteger(C0009R$integer.heads_up_default_snooze_length_ms));
    ...
    public void snooze() {
        for (String str : this.mAlertEntries.keySet()) {
            HeadsUpEntry headsUpEntry = getHeadsUpEntry(str);
            this.mSnoozedPackages.put(snoozeKey(headsUpEntry.mEntry.getSbn().getPackageName(), this.mUser), Long.valueOf(this.mClock.currentTimeMillis() + ((long) this.mSnoozeLengthMs)));
            super.snoozeAlertEntry(headsUpEntry);
        }
    }
