...
public class DragDownHelper implements Gefingerpoken {
    ...
    private float mEmptyDragAmountCache;
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        if (actionMasked == 1) {
            checkStopDragging();
            if (this.mFalsingManager.isUnlockingDisabled() || isFalseTouch() || !this.mDragDownCallback.onDraggedDown(this.mStartingChild, (int) (y - this.mInitialTouchY))) {
                stopDragging();
                return false;
            }
            ...
            if (expandableView2 != null) {
                handleExpansion(this.mLastHeight, expandableView2);
            } else {
                float f2 = this.mLastHeight;
                this.mEmptyDragAmountCache = f2;
                this.mDragDownCallback.setEmptyDragAmount(f2);
            }
    ...
    private void captureStartingChild(float f, float f2) {
        if (this.mStartingChild == null && this.mEmptyDragAmountCache == 0.0f) {
    ...
    private void cancelExpansion() {
        ...
        ofFloat.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() { // from class: com.android.systemui.statusbar.-$$Lambda$DragDownHelper$q6x0oNk24uuvhTw3d_iOE5k6pV4
    ...
    public /* synthetic */ void lambda$cancelExpansion$0$DragDownHelper(ValueAnimator valueAnimator) {
        this.mEmptyDragAmountCache = ((Float) valueAnimator.getAnimatedValue()).floatValue();
    ...
    private void checkStopDragging() {
        if (this.mStartingChild != null && this.mEmptyDragAmountCache != 0.0f) {
            cancelExpansion();
        }
    }
