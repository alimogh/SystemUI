...
public final class ControlHolderAccessibilityDelegate extends AccessibilityDelegateCompat {
    private static final int MOVE_AFTER_ID = C0008R$id.accessibility_action_controls_move_after;
    private static final int MOVE_BEFORE_ID = C0008R$id.accessibility_action_controls_move_before;
    ...
    public ControlHolderAccessibilityDelegate(@NotNull Function1<? super Boolean, ? extends CharSequence> function1, @NotNull Function0<Integer> function0, @Nullable ControlsModel.MoveHelper moveHelper) {
    ...
    private final void addClickAction(View view, AccessibilityNodeInfoCompat accessibilityNodeInfoCompat) {
        String str;
        if (this.isFavorite) {
            str = view.getContext().getString(C0015R$string.accessibility_control_change_unfavorite);
        } else {
            str = view.getContext().getString(C0015R$string.accessibility_control_change_favorite);
        }
    ...
    private final void maybeAddMoveBeforeAction(View view, AccessibilityNodeInfoCompat accessibilityNodeInfoCompat) {
        ...
        if (moveHelper != null ? moveHelper.canMoveBefore(this.positionRetriever.invoke().intValue()) : false) {
            accessibilityNodeInfoCompat.addAction(new AccessibilityNodeInfoCompat.AccessibilityActionCompat(MOVE_BEFORE_ID, view.getContext().getString(C0015R$string.accessibility_control_move, Integer.valueOf((this.positionRetriever.invoke().intValue() + 1) - 1))));
    ...
    private final void maybeAddMoveAfterAction(View view, AccessibilityNodeInfoCompat accessibilityNodeInfoCompat) {
        ...
        if (moveHelper != null ? moveHelper.canMoveAfter(this.positionRetriever.invoke().intValue()) : false) {
            accessibilityNodeInfoCompat.addAction(new AccessibilityNodeInfoCompat.AccessibilityActionCompat(MOVE_AFTER_ID, view.getContext().getString(C0015R$string.accessibility_control_move, Integer.valueOf(this.positionRetriever.invoke().intValue() + 1 + 1))));
            accessibilityNodeInfoCompat.setContextClickable(true);
        }
    }
}
