...
public class ForcedResizableInfoActivityController {
    ...
    private final Consumer<Boolean> mDockedStackExistsListener = new Consumer() { // from class: com.android.systemui.stackdivider.-$$Lambda$ForcedResizableInfoActivityController$54i4Imkkt5mUAQHgG0VrHpWhZ10
    ...
    private void activityForcedResizable(String str, int i, int i2) {
        if (!debounce(str)) {
            ...
            Slog.d("ForcedResizableInfoActivityController", "do not add " + str + " in pending task since it is in quick reply mode. taskId=" + i + " reason=" + i2);
        }
    }
    ...
    private void activityDismissingDockedStack() {
        Toast makeText = Toast.makeText(this.mContext, C0015R$string.dock_non_resizeble_failed_to_dock_text, 0);
    ...
    private void activityLaunchOnSecondaryDisplayFailed() {
        Toast.makeText(this.mContext, C0015R$string.activity_launch_on_secondary_display_failed_text, 0).show();
    }
