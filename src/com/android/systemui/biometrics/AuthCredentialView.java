...
public abstract class AuthCredentialView extends LinearLayout {
    ...
    protected static class ErrorTimer extends CountDownTimer {
        ...
        public void onTick(long j) {
            this.mErrorView.setText(this.mContext.getString(C0015R$string.biometric_dialog_credential_too_many_attempts, Integer.valueOf((int) (j / 1000))));
        }
    }
    ...
    public void onAttachedToWindow() {
        ...
        if (this.mIconView != null) {
            if (Utils.isManagedProfile(((LinearLayout) this).mContext, this.mEffectiveUserId)) {
                drawable = getResources().getDrawable(C0006R$drawable.auth_dialog_enterprise, ((LinearLayout) this).mContext.getTheme());
            } else {
                drawable = getResources().getDrawable(C0006R$drawable.auth_dialog_lock, ((LinearLayout) this).mContext.getTheme());
            }
            this.mIconView.setImageDrawable(drawable);
        }
        if (this.mShouldAnimateContents) {
            setTranslationY(getResources().getDimension(C0005R$dimen.biometric_dialog_credential_translation_offset));
            setAlpha(0.0f);
            postOnAnimation(new Runnable() { // from class: com.android.systemui.biometrics.-$$Lambda$AuthCredentialView$KVtRMfNSJ6YMQd7FjO_ZTh576v4
                @Override // java.lang.Runnable
    ...
    public void onFinishInflate() {
        ...
        this.mTitleView = (TextView) findViewById(C0008R$id.title);
        this.mSubtitleView = (TextView) findViewById(C0008R$id.subtitle);
        this.mDescriptionView = (TextView) findViewById(C0008R$id.description);
        this.mIconView = (ImageView) findViewById(C0008R$id.icon);
        this.mErrorView = (TextView) findViewById(C0008R$id.error);
    }
    ...
    public void onCredentialVerified(byte[] bArr, int i) {
        ...
        if (bArr != null) {
            ...
            AnonymousClass2 r8 = new ErrorTimer(((LinearLayout) this).mContext, this.mLockPatternUtils.setLockoutAttemptDeadline(this.mEffectiveUserId, i) - SystemClock.elapsedRealtime(), 1000, this.mErrorView) { // from class: com.android.systemui.biometrics.AuthCredentialView.2
                @Override // android.os.CountDownTimer
            ...
            if (i3 == 1) {
                i2 = C0015R$string.biometric_dialog_wrong_pin;
            } else if (i3 != 2) {
                i2 = C0015R$string.biometric_dialog_wrong_password;
            } else {
                i2 = C0015R$string.biometric_dialog_wrong_pattern;
            }
    ...
    private boolean updateErrorMessage(int i) {
        ...
        if (this.mErrorView != null) {
            showError(getResources().getString(C0015R$string.biometric_dialog_credential_attempts_before_wipe, Integer.valueOf(i), Integer.valueOf(maximumFailedPasswordsForWipe)));
        }
    ...
    private void showLastAttemptBeforeWipeDialog() {
        AlertDialog create = new AlertDialog.Builder(((LinearLayout) this).mContext).setTitle(C0015R$string.biometric_dialog_last_attempt_before_wipe_dialog_title).setMessage(getLastAttemptBeforeWipeMessageRes(getUserTypeForWipe(), this.mCredentialType)).setPositiveButton(17039370, (DialogInterface.OnClickListener) null).create();
        create.getWindow().setType(2017);
        create.show();
    }

    private void showNowWipingDialog() {
        AlertDialog create = new AlertDialog.Builder(((LinearLayout) this).mContext).setMessage(getNowWipingMessageRes(getUserTypeForWipe())).setPositiveButton(C0015R$string.biometric_dialog_now_wiping_dialog_dismiss, (DialogInterface.OnClickListener) null).setOnDismissListener(new DialogInterface.OnDismissListener() { // from class: com.android.systemui.biometrics.-$$Lambda$AuthCredentialView$BXXne-WVQqWIG1IKb-D_tTyLfJQ
    ...
    private static int getLastAttemptBeforeWipeDeviceMessageRes(int i) {
        if (i == 1) {
            return C0015R$string.biometric_dialog_last_pin_attempt_before_wipe_device;
        }
        if (i != 2) {
            return C0015R$string.biometric_dialog_last_password_attempt_before_wipe_device;
        }
        return C0015R$string.biometric_dialog_last_pattern_attempt_before_wipe_device;
    }

    private static int getLastAttemptBeforeWipeProfileMessageRes(int i) {
        if (i == 1) {
            return C0015R$string.biometric_dialog_last_pin_attempt_before_wipe_profile;
        }
        if (i != 2) {
            return C0015R$string.biometric_dialog_last_password_attempt_before_wipe_profile;
        }
        return C0015R$string.biometric_dialog_last_pattern_attempt_before_wipe_profile;
    }

    private static int getLastAttemptBeforeWipeUserMessageRes(int i) {
        if (i == 1) {
            return C0015R$string.biometric_dialog_last_pin_attempt_before_wipe_user;
        }
        if (i != 2) {
            return C0015R$string.biometric_dialog_last_password_attempt_before_wipe_user;
        }
        return C0015R$string.biometric_dialog_last_pattern_attempt_before_wipe_user;
    }

    private static int getNowWipingMessageRes(int i) {
        if (i == 1) {
            return C0015R$string.biometric_dialog_failed_attempts_now_wiping_device;
        }
        if (i == 2) {
            return C0015R$string.biometric_dialog_failed_attempts_now_wiping_profile;
        }
        if (i == 3) {
            return C0015R$string.biometric_dialog_failed_attempts_now_wiping_user;
        }
        throw new IllegalArgumentException("Unrecognized user type:" + i);
    }
